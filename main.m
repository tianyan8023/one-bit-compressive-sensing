...
for var = start:gap:final	  
    ...
    for ii = 1:times   
		...
		% Use OB-CS algorithm to reconstruct feature vector w 
        % [w] = OBCS(x_train,y_train,tau_w,K,l_max);
        % Use OB-PCS algorithm to reconstruct feature vector w 
        % [w] = OBPCS(x_train,y_train,tau_w,tau_d,K,l_max,t_max);	
        % Use OB-PBCS algorithm to reconstruct feature selection vector w
